...
Primary Run #93 (`33621678436`), attempt 2, job `100256100825`, commit `4a8cae2ee848c86c01f13c0452ff014df35ace19`, passed the WeasyPrint staging and isolated direct/relocation gate plus all earlier dependency gates through bundled Java, then failed because the real-backend step did not detect the dynamically assigned server port from package-local logs inside the existing 150-second window. The snapshot retained after failure showed package-local Java and desktop/WebView2 activity and actual backend log files, so do not classify this as an immediate WeasyPrint executable crash. The old collector omitted those logs and generated a recursive portable-tree file larger than the 2 MB diagnostic cap, creating a secondary diagnostic-step failure.

Diagnostic correction: `.github/scripts/collect-startup-diagnostics.ps1` replaces the recursive full-package dump with bounded backend-log tails plus concise host/data/WebView2/layout/process evidence, includes `weasyprint` in process capture, prunes optional snapshots if necessary, and keeps the 2 MB / 3-day retention contract. The primary preflight parses this script. This is observability-only: do not change the backend timeout, WeasyPrint payload/shim, accepted dependency versions, or functional gates until the next primary run yields the exact backend log cause. Do not start Calibre meanwhile.
