...
- Status: **portable bootstrap/backend/WebView2-profile containment proven; run #50 proved package-local window-state persistence but exposed second-launch restore timing plus a real Roaming `connection.json` leak; the current fix localizes the store/log targets and moves restore to Tauri `on_window_ready`, pending CI proof; native HTTP `.cookies` remains a known LocalAppData leak to localize before final Release**
...
`tauri-plugin-window-state` 2.2.1 cannot be pointed at a portable directory: it always writes through Tauri's `app_config_dir()` (Roaming AppData). PDF_Tunner therefore keeps the official plugin unchanged outside portable mode, but portable mode substitutes a localized equivalent under `data/tauri/window-state/.window-state.json`. The replacement keeps the upstream state shape (`width`, `height`, `x`, `y`, `prev_x`, `prev_y`, `maximized`, `visible`, `decorated`, `fullscreen`), avoids restoring a saved rectangle that no longer intersects an available monitor, captures the last window before it disappears, and saves the state before PDF_Tunner's terminal `cleanup_before_exit()`/`process::exit()` sequence. Run #50 proved that the first launch really writes the requested measured geometry package-locally (client 824x581 at x=111/y=87), but the second launch ended around 1028x749 at x=0/y=0. The main Tauri config has no minimum width/height, so that smaller test geometry is valid. The portable implementation now mirrors the official plugin lifecycle more closely: it loads the cache during plugin setup and restores/tracks each fully-created main or dynamic window from `on_window_ready`, rather than restoring during application `setup()` where later native window realization can overwrite geometry. This change remains unproven until the two-launch CI validator passes.

Run #50 and the later startup diagnostics also proved that Stirling's desktop connection settings do use `tauri-plugin-store`: relative `connection.json` was persisted under `%APPDATA%\com.willsitogg.pdf-tunner`. Portable mode now resolves that same store to the absolute package-local path `data/tauri/store/connection.json`; non-portable behavior remains `connection.json` through Tauri AppData.

`tauri-plugin-http` 2.5.8 is still registered with its normal cookie feature because Stirling's desktop HTTP client genuinely uses it and supports credentialed requests. The official plugin hard-wires its persistent cookie jar to `app_cache_dir()/.cookies`, which on Windows creates `%LOCALAPPDATA%\com.willsitogg.pdf-tunner\.cookies`. This is a **known unresolved portable leak**: it must be localized without disabling cookies/authentication. The exact official source ref `tauri-apps/plugins-workspace` `http-v2.5.8` has been verified; a minimal source-compatible localization will be prepared separately and must be CI-tested before final Release.
...
- package-local Tauri store/log state rather than Roaming `connection.json` or LocalAppData Tauri log files;
...
Run #50 (`run_id 32582638502`) then reached the genuine second-launch proof. The first launch measured client 824x581 at x=111/y=87 and saved exactly those values package-locally. On relaunch the main window appeared around 1028x749 at x=0/y=0, so restore failed. Inspection confirmed neither the base nor PDF_Tunner Tauri config imposes a main-window minimum that would invalidate 824x581. The official Tauri window-state plugin restores from `on_window_ready`, while PDF_Tunner had been restoring during app `setup()`. The current revision therefore moves portable restore/tracking to `on_window_ready` and removes duplicate manual tracking from dynamic-window creation; this is the behavior CI must now prove.

The later push run exposed through the connector bridge as `run_id 32598488359` failed earlier in the startup containment step, but its diagnostics proved the Java backend itself started correctly on port 53150 and shut down cleanly. The actual failure was `%APPDATA%\com.willsitogg.pdf-tunner\connection.json`. The same artifact also classified `%LOCALAPPDATA%\com.willsitogg.pdf-tunner\logs\PDF_Tunner.log` as the default `tauri-plugin-log` target and `%LOCALAPPDATA%\com.willsitogg.pdf-tunner\.cookies` as `tauri-plugin-http`'s native cookie jar. The first two host writes are addressed in the current revision; the cookie jar remains the next containment fix.
