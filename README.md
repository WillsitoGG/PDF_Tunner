...
- extraction of the expected searchable text layer using OCRmyPDF's already-packaged `pdfminer-six` dependency;
...
Focused Run `33167136751` (#4), job `98835125870`, advanced one step further: Python/OCRmyPDF staging, hashes, canonical dependency inventory and `pip check` all passed, and the real OCRmyPDF command was invoked with the corrected 300-DPI declaration. OCRmyPDF then rejected the generated PNG because the default `System.Drawing.Bitmap` carried an alpha channel. The fixture is now constructed explicitly as `Format24bppRgb`, preserving a normal opaque scan image without changing the packaged OCR runtime. In parallel, primary Run `33167136777` (#75) remained fully green, confirming no regression in previously accepted layers while the focused OCR gate is iterated.

The searchable-text assertion deliberately reuses `pdfminer-six`, which OCRmyPDF 17.10.0 already installs, instead of introducing `pypdf` solely for CI validation. This keeps the test evidence inside the actual candidate dependency set.
