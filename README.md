...
## Active diagnostic continuation — WeasyPrint Run #94

Primary Run #94 (`33666446582`), job `100369276881`, commit `3afffbc52eb2450eede8ea112ce0628a0bd8b3c4`, again failed in `Start PDF_Tunner and validate real backend`, but the corrected bounded collector succeeded and retained artifact `9861833508` (`PDF_Tunner-startup-diagnostics`, 23,595 bytes, digest `sha256:27b7957e29c3f168144ac6fc1da9ef39e3c6aa8635637e5a6c34a9804e18926c`, expires 2026-09-05). The retained logs prove that the backend itself started successfully on dynamic port `54686`, reported `Stirling-PDF Started`, and accepted the real WeasyPrint HTML→PDF and Markdown→PDF requests.

The actual #94 failure is a validator-only semantic mismatch. Stirling logs the probed dependency as `WeasyPrint 69.0.0 meets minimum 58.0.0`, while `.github/scripts/validate-weasyprint.ps1` required the shorter literal `WeasyPrint 69.0 meets minimum 58.0`. The same logs contain real `Running command: weasyprint` executions for both backend routes, so there is no evidence of a WeasyPrint runtime, shim, PATH, backend-startup or conversion failure.

The targeted correction keeps the pinned `69.0` payload, archive hash, CLI, isolated PATH, real conversions, relocation, backend endpoints, timeout and every earlier gate unchanged. Only the backend-log assertion now accepts Stirling's patch-normalized `.0` rendering for the already pinned candidate/minimum identities. WeasyPrint remains **candidate/pending** until the next complete primary regression is green; do not start Calibre before that evidence exists.
