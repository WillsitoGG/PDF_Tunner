...
The validator keeps every existing Python/OCRmyPDF/NumPy gate and additionally proves package-local `cv2` resolution, the exact installed PyPI distribution version `4.14.0.94`, the exact OpenCV runtime/core version reported by `cv2.__version__` (`4.14.0`), AMD64 identity of all packaged `.pyd`/DLL native payloads, a synthetic real execution of Stirling's own `split_photos.py` producing two expected cropped images, repetition after relocation to a path containing spaces, and absence of `Missing dependency: Python with OpenCV` / `Disabling group: OpenCV` in live backend logs when they are present. Distribution and runtime versions are intentionally validated as separate identities because the `opencv-python` wheel build component (`.94`) is not part of `cv2.__version__`.

Primary Run #91 (`33539166188`), job `99960820047`, commit `92643f78737dc64e851156d23050a61927ba60bf`, authenticated and installed `opencv-python-headless 4.14.0.94`, staged the complete Python/OCRmyPDF/NumPy/OpenCV payload, passed `pip check` and passed the NumPy 2.5.2 package-local AMD64/matrix gate. It then stopped at the first OpenCV validator comparison because the validator incorrectly compared the PyPI distribution version `4.14.0.94` with `cv2.__version__`, which correctly returned OpenCV core/runtime `4.14.0`. This is a deterministic validator-semantic failure, not evidence of a broken OpenCV package. The failure diagnostics also contained the packaged `cv2` tree and `opencv_python_headless-4.14.0.94.dist-info`. The corrective change only separates distribution/runtime version checks; it does not change the candidate wheel, lock, hash, preparation path or any native/E2E/relocation/backend gate.
