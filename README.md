...
Focused Run `33254797382` (#4), job `99106520093`, ruled out that residual-process hypothesis: candidate evidence recorded `PRE_RELOCATION_RESIDUAL_COUNT=0` and `PRE_RELOCATION_REMAINING_COUNT=0`, yet the warm post-move conversion still returned success without producing a PDF. The next diagnostic therefore does not guess at another single cause. Before the unchanged strict validator runs, it copies the prepared portable tree to a path containing spaces and executes a relocation matrix that isolates the executable location from profile, TEMP/TMP and input/output locations (`EXTERNAL_ALL`, `PACKAGE_PROFILE`, `PACKAGE_IO`, `PACKAGE_TEMP`, `ALL_PACKAGE`). It also records whether LibreOffice exposes `program/python.exe` and, if present, whether that interpreter can import PyUNO plus pinned unoserver. The strict warm-relocation gate remains enabled and no result from this diagnostic copy constitutes acceptance.
