...
fn main() {
    let current_exe = env::current_exe().unwrap_or_else(|error| {
        fail(format!("PDF_Tunner unoconvert shim could not resolve its executable path: {error}"), 126)
    });
    let bin_dir = current_exe.parent().unwrap_or_else(|| fail("unoconvert.exe has no parent directory", 126));
    let tools_dir = bin_dir.parent().unwrap_or_else(|| fail("unoconvert.exe is not under tools/bin", 126));
    let portable_root = tools_dir.parent().unwrap_or_else(|| fail("Unable to resolve PDF_Tunner portable root", 126));
    ...
    let soffice = if soffice_com.is_file() { soffice_com } else { soffice_exe };
    if !soffice.is_file() {
        fail(format!("Bundled LibreOffice launcher is missing: {}", soffice.display()), 126);
    }

    let args: Vec<OsString> = env::args_os().skip(1).collect();
    if args.iter().any(|arg| { let text = arg.to_string_lossy(); text == "--version" || text == "-V" }) {
    ...
    if args.iter().any(|arg| { let text = arg.to_string_lossy(); text == "--help" || text == "-h" }) {
    ...
    if positionals.len() != 2 {
        fail(format!("Expected INPUT and OUTPUT, got {} positional arguments", positionals.len()), 2);
    }
    ...
    let output_parent = output.parent().filter(|p| !p.as_os_str().is_empty()).unwrap_or_else(|| Path::new("."));
    if let Err(error) = fs::create_dir_all(output_parent) {
        fail(format!("Unable to create output directory {}: {error}", output_parent.display()), 126);
    }
    ...
    let stamp = SystemTime::now()
        ...
        .unwrap_or(0);
    let profile = portable_root.join("p").join(format!("u-{}-{stamp}", process::id()));
    let temp = portable_root.join("data").join("tmp").join("libreoffice");
    if let Err(error) = fs::create_dir_all(&profile) {
        fail(format!("Unable to create package-local LibreOffice profile {}: {error}", profile.display()), 126);
    }
    if let Err(error) = fs::create_dir_all(&temp) {
        let _ = fs::remove_dir_all(&profile);
        fail(format!("Unable to create package-local LibreOffice temp {}: {error}", temp.display()), 126);
    }
    ...
    if let Some(filter) = &input_filter {
        command.arg(OsString::from(format!("--infilter={}", filter.to_string_lossy())));
    }
    command
        .arg("--convert-to")
        .arg(&convert_to)
    ...
    let status = match command.status() {
        ...
        Err(error) => {
            let _ = fs::remove_dir_all(&profile);
            fail(format!("Unable to start bundled LibreOffice through {}: {error}", soffice.display()), 126);
        }
    };
    if !status.success() {
    ...
    let stem = input.file_stem().unwrap_or_else(|| fail("Input file has no filename stem", 2));
    ...
    if !generated.is_file() {
        let _ = fs::remove_dir_all(&profile);
        fail(format!("LibreOffice exited successfully but expected output was not produced: {}", generated.display()), 1);
    }
    if let Err(error) = move_or_copy(&generated, &output) {
        let _ = fs::remove_dir_all(&profile);
        fail(format!("Unable to place converted output at {}: {error}", output.display()), 1);
    }
    let _ = fs::remove_dir_all(&profile);
}
