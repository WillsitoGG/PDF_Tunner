...
$samplePdf = Join-Path $repoRoot 'test_globalsign.pdf'
...
foreach ($required in @($samplePdf, $buildGradle, $java)) {
...
Remove-Item -LiteralPath $pdfa, $pdfaHeaders, $verificationJson, $verificationHeaders -Force -ErrorAction SilentlyContinue
...
Write-Host "PASS: embedded VeraPDF $veraPdfVersion E2E validated package-local JRE support, real PDF/A-2b conversion, /api/v1/security/verify-pdf compliance, and backend execution evidence."
