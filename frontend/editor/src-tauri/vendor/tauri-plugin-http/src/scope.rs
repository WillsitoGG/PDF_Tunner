...
mod tests {
    ...
    fn pattern_wildcard() {
        ...
        assert!(scope.is_allowed(&"http://localhost:8080/assets/file.png?width=100&height=200".parse().unwrap()));
    ...
    fn domain_wildcard() {
        ...
        assert!(scope.is_allowed(&"http://something.else/path/to/file.mp4?start=500".parse().unwrap()));
