...
use std::{path::PathBuf, sync::{mpsc::Receiver, Mutex}};
use cookie_store::{CookieStore, RawCookie, RawCookieParseError};
use reqwest::header::HeaderValue;

fn set_cookies(cookie_store: &mut CookieStore, cookie_headers: &mut dyn Iterator<Item = &HeaderValue>, url: &url::Url) {
...
fn cookies(cookie_store: &CookieStore, url: &url::Url) -> Option<HeaderValue> {
    let s = cookie_store.get_request_values(url).map(|(name, value)| format!("{name}={value}")).collect::<Vec<_>>().join("; ");
...
impl CookieStoreMutex {
    pub fn new(path: PathBuf, cookie_store: CookieStore) -> CookieStoreMutex {
        CookieStoreMutex { path, store: Mutex::new(cookie_store), save_task: Default::default() }
    }

    pub fn load<R: std::io::BufRead>(path: PathBuf, reader: R) -> cookie_store::Result<CookieStoreMutex> {
        cookie_store::serde::load(reader, |c| serde_json::from_str(c)).map(|store| CookieStoreMutex::new(path, store))
    }

    fn cookies_to_str(&self) -> Result<String, serde_json::Error> {
        let mut cookies = Vec::new();
        for cookie in self.store.lock().expect("poisoned cookie jar mutex").iter_unexpired() {
            if cookie.is_persistent() { cookies.push(cookie.clone()); }
        }
    ...
    pub fn request_save(&self) -> cookie_store::Result<Receiver<()>> {
        ...
        self.save_task.lock().unwrap().replace(CancellableTask(task));
        Ok(rx)
    }
}
...
impl Drop for CancellableTask { fn drop(&mut self) { self.0.abort(); } }
